...
template <
    ...
    class TiledCopyOut,
    class ComputeLayout>
__global__ void softmax_multistage_device(
...
void softmax_multistage(int m, int n, float *dIn, int ldIn, float *dOut, int ldOut) {
  ...
  softmax_multistage_device<<<dimGrid, dimBlock, cosize(sIn) * sizeof(float), nullptr>>>(
      probShape,
      ctaTiler,
      dIn,
      strideIn,
